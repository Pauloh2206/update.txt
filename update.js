...
async function createBackup() {
  ...
  try {
    ...
    // --- NOVO: Backup do package.json e package-lock.json ---
    const packageJsonFile = path.join(process.cwd(), 'package.json');
    const packageLockFile = path.join(process.cwd(), 'package-lock.json');
    
    if (fsSync.existsSync(packageJsonFile)) {
      printDetail('📝 Copiando package.json (COM CHAVES SECRETAS)...');
      await fs.copyFile(packageJsonFile, path.join(BACKUP_DIR, 'package.json'));
    }
    if (fsSync.existsSync(packageLockFile)) {
      printDetail('📝 Copiando package-lock.json...');
      await fs.copyFile(packageLockFile, path.join(BACKUP_DIR, 'package-lock.json'));
    }
    // --- FIM NOVO ---
    ...
    const backupPackageJson = path.join(BACKUP_DIR, 'package.json'); // NOVO: Verificação
    ...
    const backupSuccess = (
      ...
      (fsSync.existsSync(backupPackageJson)) // NOVO: package.json deve existir
    );
...
async function cleanOldFiles(options = {}) {
  ...
  try {
    const itemsToDelete = [
      ...
      // --- NOVO: Remove os arquivos package.json e package-lock.json ANTES de copiar o novo,
      // pois eles serão restaurados do backup.
    ...
    if (removePackageLock) {
      // package-lock.json já está listado acima para ser removido sempre antes de copiar o novo
    } else {
...
async function restoreBackup() {
  ...
  try {
    ...
    // --- NOVO: Restaurando package.json e package-lock.json ---
    const backupPackageJson = path.join(BACKUP_DIR, 'package.json');
    const backupPackageLock = path.join(BACKUP_DIR, 'package-lock.json');
    
    if (fsSync.existsSync(backupPackageJson)) {
      printDetail('📝 Restaurando package.json (COM CHAVES SECRETAS)...');
      await fs.copyFile(backupPackageJson, path.join(process.cwd(), 'package.json'));
    }
    if (fsSync.existsSync(backupPackageLock)) {
      printDetail('📝 Restaurando package-lock.json...');
      await fs.copyFile(backupPackageLock, path.join(process.cwd(), 'package-lock.json'));
    }
    // --- FIM NOVO ---
